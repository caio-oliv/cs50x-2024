...
void take_preferences(uint32_t voter_count, uint8_t candidate_cnt,
                      const string candts[MAX_CANDIDATES],
                      int out_preferences[MAX_CANDIDATES][MAX_CANDIDATES]);

bool real_vote(const string name, const string candts[], uint8_t candidate_cnt, int *out_vote);
void real_record_preferences(int pref[MAX_CANDIDATES][MAX_CANDIDATES], const int ranks[],
                             const uint8_t candidate_cnt);
bool is_preference_tie(const int pref[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_a,
                       const uint8_t candidate_b);
int fill_pairs(pair_t pair_arr[PAIR_CAPACITY], const int pref[MAX_CANDIDATES][MAX_CANDIDATES],
               const uint8_t candidate_cnt);
...
void graph_pairs(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES],
                 const pair_t pair_arr[PAIR_CAPACITY], const int pair_cnt,
                 const uint8_t candidate_cnt);
bool winning_graph_has_cycle(const bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES],
                             const uint8_t candidate_cnt);
int32_t get_winner(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_cnt);
bool won_over_some(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate,
                   const uint8_t candidate_cnt);
bool lose_over_some(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate,
                    const uint8_t candidate_cnt);
...
void take_preferences(const uint32_t voter_count, const uint8_t candidate_cnt,
                      const string candts[MAX_CANDIDATES],
                      int out_preferences[MAX_CANDIDATES][MAX_CANDIDATES])
{
...
bool real_vote(const string name, const string candts[], const uint8_t candidate_cnt, int *out_vote)
{
                             ...
                             const uint8_t candidate_cnt)
{
    for (uint8_t up_rank = 0; up_rank < candidate_cnt; up_rank++)
    {
        ...
        for (uint8_t low_rank = low_rank_start; low_rank < candidate_cnt; low_rank++)
        {
...
bool is_preferred_candidate(const int pref[MAX_CANDIDATES][MAX_CANDIDATES],
                            const uint8_t candidate_a, const uint8_t candidate_b)
{
    return pref[candidate_a][candidate_b] > pref[candidate_b][candidate_a];
}
...
int32_t cmp_pair(const pair_t *pair_a, const pair_t *pair_b)
{
    return (int32_t) get_pair_strength(*pair_a) - (int32_t) get_pair_strength(*pair_b);
}

void print_pair(const pair_t pair, FILE *stream)
{
    fprintf(stream, "pair {.winner = %i, .loser = %i, .winner_strength = %i}", pair.winner,
            pair.loser, get_pair_strength(pair));
}
