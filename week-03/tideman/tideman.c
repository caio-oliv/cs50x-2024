...
// Function prototypes
uint8_t validade_args(int argc, const string argv[], string out_candidates[MAX_CANDIDATES]);
...
bool vote(const string name, const string candidates[], uint8_t candidate_count, uint8_t *out_vote);
void record_preferences(uint32_t preferences[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t ranks[], const uint8_t candidate_count);
bool is_preference_tie(const uint32_t preferences[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_a, const uint8_t candidate_b);
bool is_preferred_candidate(const uint32_t preferences[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_a, const uint8_t candidate_b);
uint8_t fill_pairs(pair_t pairs[PAIR_CAPACITY], const uint32_t preferences[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_count);
...
void graph_pairs(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const pair_t pairs[PAIR_CAPACITY], const uint8_t pair_count, const uint8_t candidate_count);
bool winning_graph_has_cycle(const bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_count);
int32_t get_winner(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_count);
bool won_over_some(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate, const uint8_t candidate_count);
bool lose_over_some(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate, const uint8_t candidate_count);
...
uint8_t validade_args(const int argc, const string argv[], string out_candidates[MAX_CANDIDATES])
{
...
bool vote(const string name, const string candidates[], const uint8_t candidate_count, uint8_t *out_vote)
{
...
void record_preferences(uint32_t preferences[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t ranks[], const uint8_t candidate_count)
{
    for (uint8_t up_rank = 0; up_rank < candidate_count; up_rank++)
    {
        ...
        for (uint8_t low_rank = low_rank_start; low_rank < candidate_count; low_rank++)
        {
...
bool is_preference_tie(const uint32_t preferences[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_a, const uint8_t candidate_b)
{
    return preferences[candidate_a][candidate_b] == preferences[candidate_b][candidate_a];
}
...
uint8_t fill_pairs(pair_t pairs[PAIR_CAPACITY], const uint32_t preferences[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_count)
{
    ...
    for (uint8_t cand_a = 0; cand_a < candidate_count; cand_a++)
    {
        for (uint8_t cand_b = 0; cand_b < candidate_count; cand_b++)
        {
            ...
            {
                pairs[count] = (pair_t){.winner = cand_a, .loser = cand_b, .winner_strength = votes};
                count += 1;
            }
        }
    }
...
void print_pair(const pair_t pair, FILE *stream)
{
    fprintf(stream, "pair {.winner = %hhu, .loser = %hhu, .winner_strength = %u}", pair.winner, pair.loser, pair.winner_strength);
}
...
int32_t get_winner(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate_count)
{
    ...
    {
        if (won_over_some(winning_graph, canidate, candidate_count) && !lose_over_some(winning_graph, canidate, candidate_count))
        {
            return canidate;
        }
    }
...
bool won_over_some(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate, const uint8_t canidate_count)
{
...
bool lose_over_some(bool winning_graph[MAX_CANDIDATES][MAX_CANDIDATES], const uint8_t candidate, const uint8_t canidate_count)
{
    for (uint8_t i = 0; i < canidate_count; i++)
    {
