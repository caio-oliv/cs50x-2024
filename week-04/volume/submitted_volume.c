...
size_t read_wav_samples(int16_t *restrict wav_samples,
                        /// Number of samples in the wav_samples array
                        const size_t size, FILE *restrict stream);
size_t write_wav_samples(int16_t *restrict wav_samples,
                         /// Number of samples in the wav_samples array
                         const size_t size, FILE *restrict stream);
...
int32_t main(const int32_t argc, const string argv[])
{
    ...
    const size_t sample_block = 1024;
    int16_t *samples = box(sizeof(int16_t) * sample_block);

    size_t read = 0;
    size_t copied = 0;
    do
    {
        read = read_wav_samples(samples, sample_block, input);
        ...
        {
            copied +=
                write_wav_samples(samples + (copied * sizeof(int16_t)), read - copied, output);
            retry += 1;
        }
    }
    while (copied == sample_block);
